...
import {
  TrendingUp, Package, Users, Brain, AlertTriangle, BarChart3,
  Check, Sparkles, ShieldCheck, Zap, ArrowRight, Quote, PiggyBank, Target,
} from "lucide-react";
...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Plataforma inteligente que mostra onde você perde dinheiro e como aumentar o lucro do seu mercado, mercearia ou loja. Teste grátis 14 dias." },
      { property: "og:title", content: "Central do Comerciante" },
      { property: "og:description", content: "O sistema inteligente que mostra onde o comerciante perde dinheiro e como lucrar mais." },
    ],
  }),
  component: Landing,
});
        ...
        <div>
          ...
          <div className="mt-8 flex flex-col sm:flex-row gap-3">
            <Button asChild size="lg" className="bg-gradient-hero text-primary-foreground shadow-elegant hover:opacity-95">
              <Link to="/cadastro">Teste Grátis por 14 Dias <ArrowRight className="ml-1 h-4 w-4" /></Link>
            </Button>
...
function Benefits() {
  const items = [
    { icon: PiggyBank, t: "Lucre mais", d: "Descubra produtos e ações que aumentam sua margem em até 30%." },
    { icon: AlertTriangle, t: "Pare de perder dinheiro", d: "Identifique estoque parado, ruptura e fornecedores caros." },
    { icon: Zap, t: "Decisões em segundos", d: "Recomendações claras da IA para agir hoje, não no próximo mês." },
  ];
  return (
...
function Features() {
  const features = [
    { icon: Package, t: "Controle de Estoque", d: "Cadastro, entradas, saídas, alertas de ruptura e produtos parados." },
    { icon: Users, t: "Gestão de Fornecedores", d: "Cadastro, prazos, ranking e histórico de compras." },
    { icon: BarChart3, t: "Análise de Lucro", d: "Lucro e margem por produto, categoria e período." },
    { icon: Target, t: "Oportunidades de Lucro", d: "Painel exclusivo com economias possíveis e produtos a reajustar." },
    { icon: ShieldCheck, t: "Índice de Saúde do Lucro", d: "Score 0–100 que mostra a saúde financeira do seu negócio." },
    { icon: Brain, t: "Consultor de Lucro IA", d: "Recomendações automáticas baseadas no seu próprio negócio." },
  ];
  return (
    <section id="recursos" className="py-20 md:py-28">
      <div className="container mx-auto px-4">
        <div className="max-w-2xl mx-auto text-center mb-14">
          ...
          <h2 className="text-3xl md:text-5xl font-bold">Tudo o que você precisa para lucrar mais</h2>
          <p className="mt-4 text-muted-foreground text-lg">
            Não é mais um ERP. É um sistema feito para responder: <em>onde estou perdendo dinheiro?</em>
          </p>
        </div>
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5">
          {features.map((f) => (
            <Card key={f.t} className="p-6 bg-gradient-card hover:shadow-elegant transition-shadow border-border">
  ...
  return (
    <section className="py-20 md:py-28 bg-muted/40">
      <div className="container mx-auto px-4 grid lg:grid-cols-2 gap-12 items-center">
        <div>
          <Badge variant="secondary" className="mb-4">Comparação de fornecedores</Badge>
          <h2 className="text-3xl md:text-4xl font-bold">O mesmo produto, vários fornecedores. Compre sempre pelo melhor.</h2>
          <p className="mt-4 text-muted-foreground text-lg">
            A Central compara automaticamente seus fornecedores e mostra economia mensal e anual.
          </p>
          <ul className="mt-6 space-y-3">
            {["Melhor fornecedor por produto", "Economia percentual", "Projeção mensal e anual"].map((x) => (
        ...
        <Card className="p-6 shadow-elegant bg-gradient-card">
          ...
          <div className="space-y-2">
            {rows.map((r) => (
              <div key={r.name} className={`flex items-center justify-between rounded-lg border px-4 py-3 ${r.best ? "border-primary bg-primary/5" : "border-border"}`}>
                <span className="font-medium">{r.name}</span>
                <div className="flex items-center gap-3">
                  <span className="font-display font-bold">{r.price}</span>
                  {r.best && <Badge variant="secondary" className="text-primary">Melhor</Badge>}
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="container mx-auto px-4 grid lg:grid-cols-2 gap-12 items-center">
        <Card className="order-2 lg:order-1 p-6 bg-secondary text-secondary-foreground border-0 shadow-elegant">
          ...
          <div className="space-y-3">
            {insights.map((i, idx) => (
              <div key={idx} className="rounded-lg bg-white/5 border border-white/10 p-3 text-sm flex gap-3">
        ...
        <div className="order-1 lg:order-2">
          <Badge variant="secondary" className="mb-4">Inteligência Artificial</Badge>
          <h2 className="text-3xl md:text-4xl font-bold">Um consultor financeiro 24h analisando seu negócio.</h2>
          <p className="mt-4 text-muted-foreground text-lg">
          ...
          <Button asChild size="lg" className="mt-7 bg-gradient-hero text-primary-foreground shadow-elegant">
...
function Pricing() {
  const plans = [
    { name: "Essencial", price: "49", popular: false, features: ["Até 500 produtos", "1 usuário", "Controle de estoque", "Comparação de fornecedores", "Relatórios essenciais"] },
    { name: "Profissional", price: "99", popular: true, features: ["Até 3.000 produtos", "5 usuários", "Tudo do Essencial", "Consultor de Lucro IA", "Índice de Saúde do Lucro", "Exportação PDF/Excel"] },
    { name: "Premium", price: "149", popular: false, features: ["Produtos ilimitados", "Usuários ilimitados", "Tudo do Profissional", "Suporte prioritário", "Integrações avançadas", "Onboarding dedicado"] },
  ];
  return (
    <section id="planos" className="py-20 md:py-28 bg-muted/40">
      <div className="container mx-auto px-4">
        <div className="max-w-2xl mx-auto text-center mb-12">
          ...
          <p className="mt-4 text-muted-foreground text-lg">14 dias grátis em qualquer plano. Sem cartão de crédito.</p>
        </div>
        <div className="grid md:grid-cols-3 gap-6 max-w-5xl mx-auto">
          {plans.map((p) => (
            <Card key={p.name} className={`p-7 relative ${p.popular ? "border-primary shadow-elegant scale-[1.02] bg-gradient-card" : ""}`}>
...
function Testimonials() {
  const items = [
    { name: "Carlos Mendes", role: "Mercado do Bairro — SP", text: "Em 2 meses economizei R$ 3.800 só trocando de fornecedor nos itens que a IA apontou." },
    { name: "Joana Ribeiro", role: "Mercearia Boa Vista — MG", text: "Descobri que 18% do meu estoque estava parado. Hoje compro muito melhor." },
    { name: "Rafael Souza", role: "Adega Central — RJ", text: "O índice de saúde do lucro me deu clareza que nenhum sistema tinha dado antes." },
  ];
  return (
    <section className="py-20 md:py-28">
      <div className="container mx-auto px-4">
        <div className="max-w-2xl mx-auto text-center mb-12">
          <Badge variant="secondary" className="mb-4">Comerciantes</Badge>
...
function FAQ() {
  const faqs = [
    { q: "A Central do Comerciante é um ERP?", a: "Não. É um sistema de otimização de lucro com IA, feito para mostrar onde você perde dinheiro e como lucrar mais." },
    { q: "Como funciona o teste grátis?", a: "Você usa qualquer plano por 14 dias sem precisar de cartão de crédito. Cancele a qualquer momento." },
    { q: "Preciso ter conhecimento técnico?", a: "Não. A plataforma foi desenhada para donos de mercado, mercearia e adega sem experiência com tecnologia." },
    { q: "Posso cadastrar mais de um fornecedor por produto?", a: "Sim. É justamente assim que a comparação de fornecedores funciona, mostrando a melhor compra." },
    { q: "Meus dados estão seguros?", a: "Sim. Cada empresa tem acesso apenas aos seus próprios dados, com autenticação e criptografia." },
  ];
  return (
    <section className="py-20 md:py-28 bg-muted/40">
        ...
        <Card className="relative overflow-hidden bg-gradient-hero text-primary-foreground border-0 p-10 md:p-16 text-center shadow-elegant">
          ...
          <div className="relative">
            ...
            <p className="mt-4 opacity-90 max-w-xl mx-auto">
              14 dias grátis. Sem cartão. Sem complicação. Veja em minutos onde seu comércio pode ganhar mais dinheiro.
            </p>
