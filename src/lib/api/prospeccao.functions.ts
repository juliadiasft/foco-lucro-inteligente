...
export const listProspects = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    // As condições são montadas em pedaços com parâmetros numerados, e nunca
    // por concatenação de texto: é lista grande e filtro vindo da tela, que é
    ...
    const [total, linhas] = await Promise.all([
      ...
      query<{
        ...
        `SELECT p.id,p.cnpj,p.razao_social,p.nome_fantasia,p.cidade,p.uf,p.telefone,p.whatsapp,
                p.email,p.fornece,p.confere,p.matriz_ou_filial,p.enderecos_da_empresa,
          ...
          WHERE ${onde}
          -- Quem tem WhatsApp primeiro, depois quem é atividade principal:
          -- é a ordem em que a conversa tem mais chance de acontecer.
          ORDER BY (p.whatsapp <> '') DESC,
...
export const getProspectResumo = createServerFn({ method: "POST" })
  .validator(z.object({ lado: z.enum(LADOS).default("fornecedor"), nicho: z.string().default("pet") }))
  .handler(async ({ data }) => {
    await requireStaff();
...
export const listProspectCidades = createServerFn({ method: "POST" })
  .validator(z.object({ lado: z.enum(LADOS), nicho: z.string().default("pet"), uf: z.string().length(2) }))
  .handler(async ({ data }) => {
    await requireStaff();
